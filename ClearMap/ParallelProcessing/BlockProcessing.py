...
def process(
    ...
    **kwargs,
    ...
    split = ft.partial(
        ...
        size_max=size_max,
        size_min=size_min,
        size_multiple_of=size_multiple_of,
        overlap=overlap,
...
def block_sizes(
    ...
    if overlap is None:
        overlap = 0
    if overlap >= size:
        warnings.warn("reducing the overlap to size-1, overlap was >= size")
        overlap = size - 1
...
def block_axes(source, axes=None):
    ...
    if axes == "all" or axes is all:  # FIXME: remove is all
        if axes is all:
            warnings.warn(
                "Parameter axes could take all (the built-in Python keyword). "
...
def split_into_blocks(
    ...
    **kwargs,
    ...
    for d in range(ndim):
        if d in axes:
            n_blocks, block_ranges, valid_ranges = block_sizes(
                shape[d],
                processes=processes,
                size_max=size_max[a],
                size_min=size_min[a],
                ...
                optimization_fix=optimization_fix[a],
                verbose=verbose,
            )
            a += 1
        else:
        ...
        offsets = [
            (v[0] - b[0], b[1] - v[1]) if b != (None, None) else (None, None)
            for b, v in zip(block_ranges, valid_ranges)
        ]
